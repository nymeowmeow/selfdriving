...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    K = p_cov_check @ h_jac.T @ inv(h_jac @ p_cov_check @ h_jac.T + sensor_var * I3)

    # 3.2 Compute error state
    error_state = K @ (y_k - p_check)

    # 3.3 Correct predicted state
    p_check += error_state[:3]
    v_check += error_state[3:6]
    q_check = Quaternion(axis_angle=error_state[6:]).quat_mult(q_check)

    # 3.4 Compute corrected covariance
    p_cov_check = (I9 - K @ h_jac) @ p_cov_check
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + v_est[k-1] * delta_t + (temp - g) * delta_t**2 / 2
    ...
    if lidar_found:
    #if lidar_index < lidar.t.shape[0] and lidar.t[lidar_index] == imu_f.t[k-1]:
        p_est[k], v_est[k], q_est[k], p_cov[k]= measurement_update(var_lidar,
               p_cov[k], lidar.data[lidar_index].T, p_est[k], v_est[k], q_est[k])

    gnss_found = False
    while gnss_index < gnss.t.shape[0]:
        if abs(gnss.t[gnss_index] - imu_f.t[k]) < threshold:
            gnss_found = True
            break
        elif gnss.t[gnss_index] > imu_f.t[k] + threshold:
            break
        gnss_index += 1
        
    if gnss_found:
    #if gnss_index < gnss.t.shape[0] and gnss.t[gnss_index] == imu_f.t[k-1]:
        p_est[k], v_est[k], q_est[k], p_cov[k]= measurement_update(var_gnss,
              p_cov[k], gnss.data[gnss_index].T, p_est[k], v_est[k], q_est[k])
