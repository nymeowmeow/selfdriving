...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
            stop_index, stop_sign_found = self.check_for_stop_signs(waypoints, closest_index, goal_index)
            self._goal_index = stop_index if stop_sign_found else goal_index
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        #   arc_length += ...
        # ------------------------------------------------------------------
        for i in range(wp_index + 1, len(waypoints)):
            arc_length += np.linalg.norm([waypoints[i][0] - waypoints[i-1][0], waypoints[i][1] - waypoints[i-1][1]])
            if arc_length > self._lookahead:
                break
        return i
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        d = np.linalg.norm([ego_state[0] - waypoints[i][0], ego_state[1] - waypoints[i][1]])
        if d < closest_len:
            closest_len = d
            closest_index = i
    #   ...
