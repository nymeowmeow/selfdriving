...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # The objective and gradient are given to you by self.objective, ands
        ...
        res = scipy.optimize.minimize(self.objective, p0, method = 'L-BFGS-B',
                                      jac = self.objective_grad, constraints = bounds)
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        thetas = a*s + (b/2)*s**2 + c*np.power(s, 3)/3 + d*np.power(s, 4)/4
        return thetas
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
